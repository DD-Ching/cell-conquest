...
Status: **P1 DONE ✅ (2026-06-10) — building.** P0 foundation + P1 (flight + render on GPU) both landed and verified in-browser (Chrome + headless Chromium, 128MB storage limit). Behind `?gpu=1`, CPU path is the untouched fallback. **Verified:** 200,000 GPU-resident drones banking at a locked 60 fps / ~0 ms sim; a Hold-Fire (H) release unleashes an UNCAPPED GPU swarm that crushes an enemy node (49→~0). What's live now:
- **P1a — instanced render:** the whole swarm drawn in ONE instanced draw straight from the SoA buffers, on a transparent overlay canvas over `#game`. `js/gpu/drone-render.js`. No readback, no per-drone JS render. `drawDroneFleets` stands aside when active.
- **P1b — GPU-resident swarm + flight:** `js/gpu/swarm.js` — drones that live ONLY on the GPU (no JS object each, growable ×1.5, NO cap). Movement is a faithful WGSL port of `steerDrone` banking; one compute dispatch flies the whole swarm; `setSwarmTarget` retargets all of it in one fill (the strike-group handle in miniature).
- **P1c — detonation → CPU damage (an early slice of P3/P4):** the move shader detonates a drone on node arrival (clears ALIVE, `atomicAdd` into a per-node hit counter); the CPU reads that compact fixed array back async (~1-frame latency, never positions) and applies node damage. Render skips dead drones. Wired to the player's Hold-Fire release (`launchGpuSwarmStrike` in `drones.js`), scaled by the banked stockpile.

**Known housekeeping before default-ON:** (1) the GPU-resident pool's `count` is a high-water mark — detonated slots stay (cheap: compute early-returns, render degenerates) but memory grows over a long session of strikes → needs compaction / a free-list (a P2 chore, NOT a cap). (2) HP bars + turret-target damage aren't on the GPU path yet (turret combat is P3). (3) render-worker + `?gpu=1` combo is gated off. Until those are handled it stays opt-in behind `?gpu=1`.

Next: P2 (GPU spatial-grid hunt — removes the H-salvo spike) and P3 (combat *against* drones on GPU). Locked decisions:
...
- **P1 — Movement + render on GPU. ✅ DONE (2026-06-10).** Ported the `steerDrone` banking model to a WGSL compute pass (`js/gpu/swarm.js`); drones drawn **instanced straight from the buffer** (`js/gpu/drone-render.js`, overlay canvas) — no per-drone JS render. Built a GPU-RESIDENT pool (no JS object per drone ⇒ truly unbounded; verified 200k @ 60fps). Targeting CPU-assigned (a node id per drone). Folded in an early slice of P3/P4: GPU node-detonation → per-node atomic hit counter → async readback → CPU node damage, wired to the Hold-Fire release as an uncapped strike. Remaining on the swarm: dead-slot compaction, turret-target damage (P3), GPU hunt (P2).
